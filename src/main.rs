		...
		if arg.starts_with("-") {
			...
			match arg.as_str() {
				"init" => {
					...
					res.insert("subCommand", "init");
					if let Some(project_name) = args.next() {
						res.insert("projectName".to_string(), project_name);
						if let Some(sdk_version) = args.next() {
							res.insert("sdkVersion".to_string(), sdk_version);
						} else {
							res.insert("sdkVersion".to_string(), "latest".to_string());
						}
					} else {
						Log::new(LogType::Err, "未指定项目名称。", 12).print();
					}
				}
				"install" => {
